   ...
   Callback = function(Value)
      if Value then
         LocalPlayer:GetAttributeChangedSignal("Steals"):Connect(function()
            if LocalPlayer:GetAttribute("Steals") and LocalPlayer:GetAttribute("Steals") > 0 then
               ...
               ShootConn = RunService.RenderStepped:Connect(function()
                  ...
                  for _,plr in ipairs(Players:GetPlayers()) do
                     if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
                        and not LocalPlayer:IsFriendsWith(plr.UserId) then
                  ...
                  if nearest and nearest.Character and nearest.Character:FindFirstChild("HumanoidRootPart") then
                     local tool = LocalPlayer.Backpack:FindFirstChild("Lazer Cape") or LocalPlayer.Character:FindFirstChild("Lazer Cape")
                     if tool then
                        ...
                        elseif tool:FindFirstChild("RemoteEvent") then
                           tool.RemoteEvent:FireServer(nearest.Character.HumanoidRootPart.Position, nearest.Character.HumanoidRootPart)
                        end
                     end
                  end
               end)
      ...
      if Value then
         UserInputService.InputBegan:Connect(function(input,gp)
            ...
            if input.UserInputType == Enum.UserInputType.MouseButton1 and AimbotReady then
               ...
               if nearest then
                  local tool = LocalPlayer.Backpack:FindFirstChild("Web Sling") or LocalPlayer.Character:FindFirstChild("Web Sling")
                  if tool then
                     LocalPlayer.Character.Humanoid:EquipTool(tool)
                     tool:Activate()
                  end
      ...
      for _,plr in ipairs(Players:GetPlayers()) do
         if plr ~= LocalPlayer and plr.Character then
            ...
            if Value then
               ...
               nameLabel.TextColor3 = LocalPlayer:IsFriendsWith(plr.UserId) and Color3.fromRGB(0,255,0) or Color3.fromRGB(255,0,0)
   ...
   Callback = function()
      ...
      local function checkWin()
         ...
         for _,combo in ipairs(wins) do
            if board[combo[1]] and board[combo[1]]==board[combo[2]] and board[combo[2]]==board[combo[3]] then
               return true
            end
         end
         return false
