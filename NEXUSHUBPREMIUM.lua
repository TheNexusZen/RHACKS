...
local Window = WindUI:CreateWindow({
    Title = "NexusHub (Premium)",
    Icon = "door-open",
    Author = "Made By NexusZen",
    Folder = "NexusPremiumConfig",
...
if not isPremium then
    plr:Kick("Script Has Been Deleted")
    return
end
...
local Input = SerTab:Input({
    ...
    Callback = function(input) jobIdInputValue = input end
})
...
SerTab:Toggle({
    ...
    Default = false,
    Callback = function(state) autoLoadScript = state end
})
...
local ServerDropdown = SerTab:Dropdown({
    ...
    Value = "High Players",
    Callback = function(option) serverFilter = option end
})
...
local function getServers(cursor)
    ...
    if success and data and data.data then return data else return nil end
end
...
local TeleportDown = tptab:Dropdown({
    ...
    Value = "",
    Callback = function(option) selectedTarget = option end
})
local RefreshBtn = tptab:Button({
    Title = "Refresh List",
    Desc = "Update player dropdown",
    Locked = false,
    Callback = function() TeleportDown:Refresh(getPlayerList()) end
})
...
local TpDropdown = tptab:Dropdown({
    ...
    Values = {},
    Value = nil,
    Callback = function(option) currentSlot = option end
})

local function refreshDropdown()
    local names = {}
    for i = 1, #savedSlots do names[i] = "Slot "..i end
    TpDropdown:Refresh(names)
    if #names > 0 then currentSlot = names[#names] else currentSlot = nil end
end
